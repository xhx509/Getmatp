...
version 0.0.2 update on 06/01/2020
1, combine rockblock 7 and ap3 together, need to change control file to rock or ap3
...
while True:
    ...
    try:
            ...
            if transmitter<>'ap3':
                if '15:20:00'<str(df2.index[-1].time())<'15:35:00' or '22:40:00'<str(df2.index[-1].time())<'22:55:00':
                    print 'sending daily time report'
                    ports='/dev/tty-huanxintrans'
                    message=str(lat_1)+','+str(lon_1)+',0000000000'
                    MoExample (ports,message)
                    print 'daily time report is sent'
                    time.sleep(1000)
                        ...
                        try:
                                ...
                                try:
                                    if transmitter=='rock':
                                            ...
                                            ports='/dev/tty-huanxintrans'
                                            message=str(lat_1)+','+str(lon_1)+','+meandepth+rangedepth+timerange+meantemp+sdeviatemp+'eee'+MAC_FILTER[0][-5:-3]+MAC_FILTER[0][-2:]+daily_ave+'t'
                                            MoExample (ports,message)
                                    ...
                                    try:
                                            if boat_type=='mobile':
                                                ...
                                                try:
                                                    ...
                                                    #print "gps parsing back to work"
                                                    #lat.append(float(df2[str(min(df2[inx:].index,key=lambda d: abs(d-i)))][1].values[0][:-1]))
                                                    #lon.append(float(df2[str(min(df2[inx:].index,key=lambda d: abs(d-i)))][2].values[0][:-1]))
                                            else:
                                                    lat.append(lat_1)
